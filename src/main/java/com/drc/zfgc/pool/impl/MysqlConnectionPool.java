...
import com.drc.zfgc.utils.DBHelper;
...
public class MysqlConnectionPool implements IConnectionPool {
    ...
    public MysqlConnectionPool(int initSize, int maxSize, int idleCount, long waitTime) {
    ...
    public Connection getConnection() {
        ...
        if (conn != null) {
            busy.offer(conn);
            logger.info("[" + Thread.currentThread().getName() + "]" +idle+ " ************从空闲队列获取连接************");
            return conn;
        }
    ...
    public void recycle(Object conn) {
        ...
        if (busy.remove(conn)) {//第一步，移除
            ...
            if (idle.size() >= idleCount) {
                DBHelper.closeConnection((Connection) conn);
                //关闭连接，则活跃连接减一
                activeSize.decrementAndGet();
    ...
    public void destory() {
        ...
        while (idle.size() != 0) {
            DBHelper.closeConnection(idle.poll());
            activeSize.decrementAndGet();
        }
    }
}
