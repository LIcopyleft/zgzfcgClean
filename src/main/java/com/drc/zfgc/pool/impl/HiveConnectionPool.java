 ...
 * @date 2020/11/5
 */
public class HiveConnectionPool implements IConnectionPool {
    ...
    public HiveConnectionPool(int initSize, int maxSize, int idleCount, long waitTime) {
        init(initSize, maxSize, idleCount, waitTime);
    }

    private static Logger logger = LoggerFactory.getLogger(HiveConnectionPool.class);
    @Override
        ...
        for (int i = 0; i < initSize; i++) {
            if (activeSize.get() < maxSize) {//两重判断是为了保证线程安全
                if (activeSize.incrementAndGet() <= maxSize) {
                    //创建连接
                    Connection conn = HiveJdbcConUtil.getConnnection();
                    //加入空闲队列
                    idle.offer(conn);
                } else {
    ...
    public Connection getConnection() {
        ...
        if (conn != null) {
            busy.offer(conn);
            logger.info("[" + Thread.currentThread().getName() + "]" + " ************从空闲队列获取连接************");
            return conn;
        }
    ...
    public void recycle(Object conn) {
        ...
        if (busy.remove(conn)) {//第一步，移除
            ...
            if (idle.size() >= idleCount) {
                HiveJdbcConUtil.closeConnection((Connection) conn);
                //关闭连接，则活跃连接减一
                activeSize.decrementAndGet();
    ...
    public void destory() {
        ...
        while (idle.size() != 0) {
            HiveJdbcConUtil.closeConnection(idle.poll());
            activeSize.decrementAndGet();
        }
    }
}
