...
public class MoneyUtils {
    ...
    public static String[] floatMult = new String[]{"元", "角", "分"};
    public static Double[] floatNums = new Double[]{1D, 0.1D, 0.01D};
    ...
    public static String rmbBigToSmall(String money) {
        Double number = 0D;
        money = money.replaceAll("整", "");
        if (money.endsWith("元")) {
            money = money.replaceAll("元", "");
        }
        if (money.endsWith("圆")) {
            money = money.replaceAll("圆", "");
        }
     ...
     * @return
     */
    private static Double getNumByBig(String big) {
        Double result = 0D;
        ...
        try {
            ...
            for (int i = 0; i < floatMult.length; i++) {

                boolean contains = big.contains(floatMult[i]);
                if (contains && i == 0 && !big.endsWith(floatMult[i])) {
                    big = big.replaceAll(floatMult[i], ".");
                }
                if (contains && i == 1 && !big.endsWith(floatMult[i])) {
                    big = big.replaceAll(floatMult[i], "");
                }
                if (contains && i == 2) {
                    big = big.replaceAll(floatMult[i], "");
                ...
                /*big = big.replaceAll(floatMult[i], floatNums[i].toString());
                big = big.replaceAll(floatMult[i], floatMult[i].toString());*/

            }
            result = Double.valueOf(big);
    ...
    public static void main(String[] args) {
        ...
        System.out.println(rmbBigToSmall("壹佰贰拾陆万捌仟肆佰玖拾伍元整"));
    }
