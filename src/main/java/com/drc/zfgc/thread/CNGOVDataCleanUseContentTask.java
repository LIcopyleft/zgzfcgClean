...
/*
 * @description  日常不使用这个类去处理数据
 * @author tengchao.li
 * @date 2020/10/28
 * @param null
 * @return
 */
public class CNGOVDataCleanUseContentTask implements Runnable {

    private int beginIndex;
    private int endIndex;
    private ConfigParam config;
    static DataContentMapper mapper = SpringContextHolder.getBean("dataContentMapper");
    private static final Logger logger = LoggerFactory.getLogger(CNGOVDataCleanUseContentTask.class);


    public CNGOVDataCleanUseContentTask(int beginIndex, ConfigParam config) {
        this.beginIndex = beginIndex;
        this.config = config;
    }

    public CNGOVDataCleanUseContentTask(int beginIndex, int endIndex, ConfigParam config) {
        this.beginIndex = beginIndex;
    ...
    public void run() {
        try {
            ...
            List<CNGovDataUrl> dataContent = mapper.selectAll4(beginIndex, endIndex, "spider_71_ggzy_zgzfcgw_content_1");
            ...
            Map<Integer, CNGovDataUrl> govDataMap = new HashMap(dataContent.size());
            for (CNGovDataUrl data : dataContent) {
                sb.append(data.getUrlId() + ",");
            //    govDataMap.put(data.getUrlId(), data);
            }

            List<CNGovDataUrl> govDatas = mapper.selectByIds(config.getUnionTableName(), sb.substring(0, sb.length() - 1));

            for(CNGovDataUrl govData : govDatas){
                govDataMap.put(govData.getUrlId(), govData);
            }

            for (CNGovDataUrl content : dataContent) {
                Integer urlId = content.getUrlId();
                CNGovDataUrl data = govDataMap.get(urlId);

                if (data == null){
                    continue;
                }
                content.setItems(data.getCategory());
                content.setType(data.getNotice());

                content.setPurchaser(data.getPurchaser());
            //    content.setCategory(data.);

                content.setPubtime(data.getPubtime());
                content.setRegion(data.getRegion() == null ? data.getAddress() : data.getRegion());
                content.setUrl(data.getUrl());
                content.setTitle(data.getTitle());
                content.setAgency(data.getAgency());
                content.setHashcode(data.getHashcode());
                content.setNotice(null);


                //   data.setContent(dataDB.getContent());
            //    data.setContent(content.getContent());
                //    data.setItems(content.getCategory());
                queues.add(content);
...
//org.datanucleus.util.StringUtils.replaceAll()
