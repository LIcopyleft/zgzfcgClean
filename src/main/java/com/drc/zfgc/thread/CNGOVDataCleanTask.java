...
public class CNGOVDataCleanTask implements Runnable {

    private int beginIndex;
    private int endIndex;
    private ConfigParam config;
    static DataContentMapper mapper = SpringContextHolder.getBean("dataContentMapper");
    private static final Logger logger = LoggerFactory.getLogger(CNGOVDataCleanTask.class);


    public CNGOVDataCleanTask(int beginIndex, ConfigParam config) {
        this.beginIndex = beginIndex;
        this.config = config;
    }

    public CNGOVDataCleanTask(int beginIndex, int endIndex, ConfigParam config) {
        this.beginIndex = beginIndex;
    ...
    public void run() {
        try {
            ...
            Map<Integer, CNGovDataUrl> map = new HashMap();
            for (CNGovDataUrl data : dataContent) {

                sb.append(data.getUrlId() + ",");
                map.put(data.getUrlId(), data);

            }

            List<CNGovDataUrl> contentDatas = mapper.selectByIds(config.getUnionTableName(), sb.substring(0, sb.length() - 1));
            for (CNGovDataUrl content : contentDatas) {
                Integer urlId = content.getUrlId();
                CNGovDataUrl data = map.get(urlId);
                data.setItems(data.getCategory());
                data.setType(data.getNotice());
                data.setNotice(null);
                data.setCategory(content.getCategory());
