...
public class CleanEsCNGovTask implements Runnable {
    ...
    private static final Logger logger = LoggerFactory.getLogger(CleanEsCNGovTask.class);


    public CleanEsCNGovTask(int beginIndex, ConfigParam config) {
        this.beginIndex = beginIndex;
        this.config = config;
    }

    public CleanEsCNGovTask(int beginIndex, int endIndex, ConfigParam config) {
        this.beginIndex = beginIndex;
        this.endIndex = endIndex;
        this.config = config;
    }

    public CleanEsCNGovTask(String sqlStr , List hits) {
        this.sqlStr = sqlStr;
        this.hits = hits;

    }

    public String getContent(int urlId, List contentList) {

        for (Object obj : contentList) {
            CNGovDataEsExample data = (CNGovDataEsExample) obj;
            if (urlId == data.getUrl_id()) {
    ...
    public LinkedBlockingQueue<CNGovDataUrl> getCleanList(String sqlStr, List<T> hits) {
        LinkedBlockingQueue<CNGovDataUrl> queues = new LinkedBlockingQueue();
        List<CNGovDataUrl> govData = mapper.selectBySql(sqlStr);
        for (CNGovDataUrl data : govData) {
            CNGovDataUrl urlData = new CNGovDataUrl();
            urlData.setUrlId(data.getUrlId());
            urlData.setPurchaser(data.getPurchaser());
            urlData.setCategory(data.getCategory());
            urlData.setItems(data.getCategory());
            urlData.setNotice(data.getNotice());
            urlData.setType(data.getNotice());
            urlData.setPubtime(data.getPubtime());
            urlData.setAddress(data.getRegion());
            urlData.setUrl(data.getUrl());
            urlData.setTitle(data.getTitle());
            urlData.setAgency(data.getAgency());
            urlData.setHashcode(data.getHashcode());
            urlData.setContent(getContent(data.getUrlId(), hits));
            queues.add(urlData);
        }
    ...
    public void run() {
        try {
            ...
            *//*for (DataContentWithBLOBs data : dataContent) {
            ...
            LinkedBlockingQueue<CNGovDataUrl> queues = getCleanList(sqlStr, hits);
            //遍历 清洗 赋值
            if (queues.size() < 1) {
                ...
                if (/*list.size() == config.getInsertMax() ||*/ queues.size() == 0) {
                    ...
                    try {
                     ...
                     //   int row = mapper.insertCNCleanDataList(list, config.getInsertTableName());
                        int row = mapper.insertCNCleanDataList(list, "temp_clean_cn");
                     //   logger.info(Thread.currentThread().getName() + "==url_id over" + dataContent.get(dataContent.size() - 1).getUrlId() + "=====insert\t" + row + "行");
                        logger.info(Thread.currentThread().getName() + "==url_id over"  + "=====insert\t" + row + "行");
                    } catch (Exception e) {
                        throw e;
                    }
                    list.clear();
